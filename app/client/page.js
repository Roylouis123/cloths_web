...
const page = () => {
    ...
    const buttonss = [
        { name: 'Reschedule', type: false },
        { name: 'Cancel', type: false },
        { name: 'View', type: true }
    ];
                ...
                <div className="flex justify-between">
                    {buttons.map((button, index) => (
                        <button
                            ...
                            className={`w-20 h-12 rounded-[50px] shadow hover:bg-gradient-to-r hover:from-orange-500 hover:via-orange-500 hover:to-fuchsia-700 ${selectedButton === index ? 'text-white bg-gradient-to-r from-orange-500 via-orange-500 to-fuchsia-700' : 'text-black bg-gradient-to-r from-zinc-100 to-zinc-100 '}`}
                        >
                            {button}
                        </button>
                    ))}
                ...
                <div className="flex justify-between mt-5">
                    {buttonss.map((button, index) => (
                        <button
                            ...
                            className={`w-28 flex justify-center item-center p-3 gap-3 h-12 rounded-[50px] shadow hover:bg-gradient-to-r hover:from-fuchsia-700 hover:to-blue-950 ${selected2Button === index ? 'text-white bg-gradient-to-r   from-fuchsia-700 to-blue-950' : 'text-black bg-gradient-to-r from-zinc-100 to-zinc-100'}`}
                        >
                            {button.name}
                ...
                <div className="flex justify-between">
                    {buttons.map((button, index) => (
                        <button
                            ...
                            style={{ boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.5)' }}
                            className={`w-20 h-12 rounded-[50px] shadow hover:bg-gradient-to-r hover:from-orange-500 hover:via-orange-500 hover:to-fuchsia-700 ${selected3Button === index ? 'text-white bg-gradient-to-r from-orange-500 via-orange-500 to-fuchsia-700' : 'text-black bg-gradient-to-r from-zinc-100 to-zinc-100 '}`}
                        >
                            {button}
                        </button>
                    ))}
                ...
                <div className="flex justify-between mt-5">
                    {buttonss.map((button, index) => (
                        <button
                            ...
                            className={`w-28 flex justify-center item-center p-3 gap-3 h-12 rounded-[50px] shadow hover:bg-gradient-to-r hover:from-fuchsia-700 hover:to-blue-950 ${selected4Button === index ? 'text-white bg-gradient-to-r   from-fuchsia-700 to-blue-950' : 'text-black bg-gradient-to-r from-zinc-100 to-zinc-100'}`}
                        >
                            {button.name}
                            {button.type && (<span><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
